...
static void startByIndex(uint8_t platform_idx, uint8_t index)
{
    ...
    str_len = GET_INSTANCE_CFG_PLATFORM()->get_partition_name(platform_idx, index, &name[0]);

    memset(src, 0, STR_LEN);
    str_len = GET_INSTANCE_CFG_PLATFORM()->get_partition_src(platform_idx, index, &src[0]);

    memset(dst, 0, STR_LEN);
    str_len = GET_INSTANCE_CFG_PLATFORM()->get_partition_dst(platform_idx, index, &dst[0]);

    memset(type, 0, STR_LEN);
    str_len = GET_INSTANCE_CFG_PLATFORM()->get_partition_type(platform_idx, index, &type[0]);

    enabled = GET_INSTANCE_CFG_PLATFORM()->get_partition_enabled(platform_idx, index);
    ...
    if(enabled) {
        ...
        if(strcmp(type, "binddir") == 0) {
            GET_INSTANCE_MISC_OBJ()->create_dir_recursive(src, DEFAULT_MODE);
            GET_INSTANCE_MISC_OBJ()->create_dir_recursive(dst, DEFAULT_MODE);
            GET_INSTANCE_MISC_OBJ()->mountfs(src, dst, NULL, 0x9000, NULL);
        } else if(strcmp(type, "squashfs") == 0) {
            memset(loop, 0, STR_LEN);
            sprintf(loop, "/dev/loop%d", index);
            GET_INSTANCE_MISC_OBJ()->bind_loopdev(src, loop, 0);
	    GET_INSTANCE_MISC_OBJ()->create_dir_recursive(dst, DEFAULT_MODE);
            GET_INSTANCE_MISC_OBJ()->mountfs(loop, dst, "squashfs", 0x8001, NULL);
        } else {
            GET_INSTANCE_MISC_OBJ()->create_dir_recursive(dst, DEFAULT_MODE);
            GET_INSTANCE_MISC_OBJ()->mountfs(src, dst, type, 0, NULL);
        }
...
static void* action_start(void* arg)
{
    ...
    if(arg == NULL) {
        no_part = GET_INSTANCE_CFG_PLATFORM()->get_no_partitions(platform_idx);
